...
LRESULT CALLBACK dlgproc(HWND hwnd,UINT mess,WPARAM wparam,LPARAM lparam)
{
	...
	case WM_INITDIALOG:
		...
		SetWindowText(GetDlgItem(hwnd,IDIP),"khiemnv.dnsalias.net");
...
// 	remoteHost=gethostbyname(ipstr);
// 	int i=0;
// 	while (remoteHost->h_addr_list[i] != 0) {
// 		clientService.sin_addr.s_addr = *(u_long *) remoteHost->h_addr_list[i++];
// 	}
	get_in_addr(ipstr,clientService.sin_addr);

	char *remoteIP = inet_ntoa(clientService.sin_addr);
